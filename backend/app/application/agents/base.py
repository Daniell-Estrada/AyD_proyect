...
class BaseAgent(ABC):
	...
	def _record_metrics(self, state: AgentState, metrics: LLMUsageMetrics) -> None:
		...
		state["total_cost_usd"] = state.get("total_cost_usd", 0.0) + metrics.estimated_cost_usd
		state["total_tokens"] = state.get("total_tokens", 0) + metrics.total_tokens
		state["total_duration_ms"] = state.get("total_duration_ms", 0.0) + metrics.duration_ms
	...
	def _invoke_llm(
		...
		print("Invoking LLM with prompts:")
		print("System Prompt:", system_prompt)
		print("User Prompt:", user_prompt)
		print("use_fallback:", use_fallback)
		if self._llm_service:
			print("primary provider/model:", self._llm_service.primary_provider, self._llm_service.primary_model)
			print("fallback provider/model:", self._llm_service.fallback_provider, self._llm_service.fallback_model)
